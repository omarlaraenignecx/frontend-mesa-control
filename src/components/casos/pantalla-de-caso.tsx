...
import { leerPlantillaDeCaso } from '@/lib/correo/plantillas'
...
import { puedeReclasificar } from '@/lib/casos/reclasificacion'
...
import { AvisoMensajesNuevos } from '@/app/caso/[fila]/aviso-mensajes'
import { Conversacion } from '@/app/caso/[fila]/conversacion'
...
import { Atender } from '@/app/caso/[fila]/atender'
import { SeguimientoForm } from '@/app/caso/[fila]/seguimiento-form'
import { SubirArchivos } from '@/app/caso/[fila]/subir-archivos'
...
/**
 * La vista de un caso, para cualquiera de los dos módulos.
 *
 * Lo único que cambia entre la Mesa de Control y Atención a Siniestros es la
 * presentación: a dónde vuelve el enlace de arriba, qué datos se listan y de qué buzón
 * habla el chat. El buzón de verdad **no** lo decide este componente sino el área del
 * caso —ver `buzonDelCaso`—, así que entrar por la URL de al lado no puede hacer que
 * una respuesta salga del área equivocada.
 */
export async function PantallaDeCaso({
  modulo,
  filaTexto,
}: {
  modulo: ConfigModulo
  ...
  if (!Number.isInteger(fila) || fila < 2) {
    return (
      <main className="mx-auto max-w-2xl space-y-3 p-8">
        ...
        <Link href={modulo.rutaLista} className="text-base text-primary underline">
  ...
  const [estadoHilo, plantillaCruda, buzon] = await Promise.all([
    cargarHilo(fila, caso.folio, caso),
    leerPlantillaDeCaso(caso),
    buzonDelCaso(caso).catch(() => null),
  ])
  const plantilla = sustituirVariables(
    plantillaCruda,
    variablesDelCaso(caso, caso.folio ?? '', usuario),
  )
  ...
  const rutaDelCaso = modulo.rutaCaso(fila)
  ...
  // Solo los campos que traen dato (RF-03). Los del ramo van primero en siniestros:
  // es lo que quien atiende necesita leer antes que nada.
  const datos = (
    [
      ['Recibido', caso.marcaTemporalTexto],
      [modulo.clasificacion.columna, caso[modulo.clasificacion.campo]],
      ...(modulo.clave === 'siniestros'
        ? ([
            ['Tipo de atención', caso.tipoAtencion],
            ['Número de siniestro', caso.numeroSiniestro],
            ['Número de póliza', caso.poliza],
  ...
  return (
    ...
    <div className="min-h-full bg-background">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="space-y-1.5">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {caso[modulo.clasificacion.campo] && (
      ...
      <main className="mx-auto max-w-7xl space-y-5 px-6 py-6">
        ...
        {modulo.generaFolios && <GenerarFolios faltantes={sinFolioTotal} />}

        {/* El correo está saliendo por un buzón que no es el del módulo. Se dice aquí,
            en la pantalla donde alguien está a punto de escribir, y no solo en los
            ajustes: es el momento en que importa. */}
        {buzon?.provisional && (
          <div className="flex items-start gap-2.5 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 dark:border-amber-900 dark:bg-amber-950">
            <TriangleAlert className="mt-0.5 size-5 shrink-0 text-amber-700 dark:text-amber-400" />
            <p className="text-base">
              <span className="font-medium">
                Este caso está usando provisionalmente el buzón de la Mesa de Control.
              </span>{' '}
              Lo que escribas saldrá de {buzon.deps.correoBuzon} y las respuestas llegarán ahí,
              firmado con la ficha del ejecutivo de siniestros.{' '}
              {modulo.ajustes && (
                <Link href={modulo.ajustes.ruta} className="underline underline-offset-4">
                  Autorizar la cuenta del módulo
                </Link>
              )}
            </p>
          </div>
            ...
            <Card className="border-primary/20 shadow-md">
              ...
              <CardContent>
                <SeguimientoForm
                  ...
                  catalogos={catalogos}
                  nombreUsuario={usuario.nombreEnHoja}
                  puedeReclasificar={puedeReclasificar(modulo, caso)}
                />
              </CardContent>
            </Card>
            ...
            <Card className="shadow-sm">
              <CardHeader>
                <CardTitle className="flex items-center gap-2.5 text-xl">
                  ...
                  Conversación
                  <AvisoMensajesNuevos fila={fila} rutaDelCaso={rutaDelCaso} />
                </CardTitle>
              </CardHeader>
              <CardContent>
                <Conversacion
