...
| Identidad visual del correo | Cabecera blanca con el logo a color y una franja cian; los colores salen del archivo de la marca (`#00afc5` para franjas y bordes, `#007797` para lo que se lee, porque el cian sobre blanco no da contraste para texto). **Las dos áreas salen iguales**: quien recibe el correo es cliente de Gplus Seguros y no tiene por qué encontrarse dos marcas según a qué área escribió; el área se distingue por el rótulo. El chasis vive en `correo/envoltura.ts` y lo comparten la respuesta al caso y el reenvío de la conversación, que antes se parecían por casualidad |
| El logo dentro del correo | Un PNG de 6 KB **incrustado en el código** (`correo/marca/logo.ts`) que viaja dentro del mensaje y se pide desde el HTML con `cid:`. Ni SVG —ningún cliente de correo lo dibuja— ni una URL servida por la aplicación —Outlook bloquea las imágenes remotas por omisión, y el logo de los correos ya enviados dependería de que el sitio siga en pie—. Se regenera desde el SVG de la marca con `scripts/generar-logo-correo.ts` |
| Trato en los textos fijos | El aviso de respuesta y el pie **no tutean ni hablan de usted**: los firman las dos áreas y no tratan igual a quien les lee —la mesa tutea a las agencias, siniestros habla de usted al cliente—, así que en cualquiera de los dos tratos uno de los dos correos salía contradiciéndose a media página |
| Aviso de respuesta | Todos los correos que salen de la mesa llevan un bloque destacado antes de la firma pidiendo **responder a ese mismo mensaje** y explicando que un correo nuevo separa la respuesta del expediente. Va en el armado del correo (`avisoDeRespuesta`), no en las plantillas que edita el área: en las plantillas se podría borrar al corregir un texto, y responder en el hilo es lo que conserva el `threadId` de la fila. **No** se agregó al reenvío de la conversación, que sale como correo aparte y cuyas respuestas no entran al chat del caso |
...
12. **En un correo, el logo va en una caja `multipart/related` junto al HTML, no suelto entre los adjuntos.** La anidación es `mixed > related > alternative`: el `related` empareja la imagen con su `src="cid:…"` por el `Content-ID`, y el `mixed` de fuera solo aparece cuando hay archivos que el destinatario descarga. Cada caja se omite cuando no hace falta —un `mixed` con una sola parte adentro hace que algunos clientes anuncien un adjunto que no existe—. `Content-Disposition: inline` es lo que evita que el logo salga además como archivo al pie.
13. **El cliente de correo arma la vista previa de la bandeja con las primeras palabras del HTML.** Sin un bloque oculto al principio, la bandeja se llena de correos que dicen todos «Gplus Seguros · Mesa de Control». El bloque se oculta con `display:none`, `max-height:0`, `opacity:0` y `mso-hide:all` a la vez, porque cada cliente respeta una, y termina en un relleno de espacios invisibles para que no se cuele detrás el texto que sigue.
14. **La búsqueda de Gmail por asunto encuentra subcadenas.** Por eso el asunto del reenvío evita a propósito la frase `Seguimiento de Caso | Gplus Seguros`: si la llevara, el hilo del reenvío podría devolverse como el hilo del caso cuando se pierda el vínculo guardado en la base.
15. **No usar `prettier` sin configuración en este repo.** Sus valores por omisión (comillas dobles, punto y coma) contradicen el estilo del código y reescriben archivos completos.
16. **El servidor corre en UTC y la hoja vive en UTC−6.** Las dos hojas declaran `locale=es_MX` y `timeZone=Etc/GMT+6` (sin horario de verano, que México eliminó en 2022), pero Vercel ejecuta en UTC y `new Date()` toma la hora local del proceso. Cualquier fecha que se escriba en la hoja o se muestre a la mesa tiene que convertirse a UTC−6 explícitamente.
17. **`loading.tsx` no se vuelve a mostrar cuando solo cambian los parámetros de búsqueda** de la misma ruta, que es lo que hacen las pestañas de vista y los filtros. Para esos casos la señal la dan `useLinkStatus` dentro del `Link` y `useTransition` alrededor del `router.push`. Además `loading.tsx` dejaría de funcionar del todo si el layout raíz empezara a leer datos de runtime —`cookies()`, sesión, un fetch sin caché—: hoy es estático a propósito y la autenticación vive en el proxy y en las páginas.
18. **`valueInputOption=RAW` guarda texto, no fechas.** Las fechas del histórico de `KB` y `KD` son números de serie con formato de fecha; lo que escribe RAW queda como cadena, que no se ordena ni entra en una fórmula. Solo los dos campos de fecha se escriben con `USER_ENTERED`, para que Sheets las interprete; el resto sigue con RAW a propósito, porque `USER_ENTERED` convertiría en fórmula unas Observaciones que empiecen con `=`.
