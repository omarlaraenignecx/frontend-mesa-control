...
- [ ] **Paso 1:** test que lee `globals.css` y exige la regla para `button`,
...
- [ ] **Paso 2:** ver el test fallar.
- [ ] **Paso 3:** agregar en `@layer base`:
      `cursor: pointer` a los selectores anteriores y `cursor: not-allowed` a
      `:disabled`.
- [ ] **Paso 4:** test verde; `pnpm build`; revisión visual en `/cola` y en un caso.
- [ ] **Paso 5:** commit.
...
- [ ] **Paso 1:** reescribir `semaforo.test.ts`: un caso por valor, el vacío, un
      valor desconocido (`N/A`), y que sobreviva el viaje por JSON del caché.
- [ ] **Paso 2:** ver fallar.
- [ ] **Paso 3:** implementar.
- [ ] **Paso 4:** verde.
- [ ] **Paso 5:** actualizar las dos vistas: punto hueco cuando no hay estatus,
      etiqueta = el propio estatus, y quitar el tinte rojo de la fila (ya no
      significa "atrasado").
- [ ] **Paso 6:** `pnpm typecheck`, commit.
...
- [ ] **Paso 1:** test de `fechaCorta(marcaTemporalIso, marcaTemporalTexto)`:
...
- [ ] **Paso 2-4:** implementar y verde.
- [ ] **Paso 5:** test de `ordenarRecientes`: el más reciente primero, los sin
      fecha al final, no muta. Sustituye a `ordenarFifo`.
- [ ] **Paso 6-7:** implementar y verde.
- [ ] **Paso 8:** tabla: `semáforo · Estatus final · Atiende · Folio · Recibido ·
...
- [ ] **Paso 1:** tests: por omisión solo trámite y sin estatus; selección
...
- [ ] **Paso 2-4:** implementar y verde.
- [ ] **Paso 5:** `FiltroEstatus` en `filtros.tsx`: botón que abre un panel con
...
- [ ] **Paso 6:** los conteos de las pestañas usan el filtro por omisión.
- [ ] **Paso 7:** `pnpm typecheck`, commit.
...
- [ ] **Paso 1:** test de `asuntoDeReenvio`: incluye el folio y **no** contiene
      `PREFIJO_ASUNTO`, para que el reenvío nunca se confunda con el hilo del caso.
- [ ] **Paso 2-3:** implementar y verde.
- [ ] **Paso 4:** tests de `cadena.ts`: el resumen cuenta mensajes y adjuntos con
      ...
      texto del usuario.
- [ ] **Paso 5-6:** implementar y verde.
- [ ] **Paso 7:** `reenviarCadena`: exige folio y conversación; valida los
...
- [ ] **Paso 8:** `reenviar-cadena.tsx`: botón junto al de enviar y modal con el
...
- [ ] **Paso 9:** `pnpm typecheck`, `pnpm build`, prueba real de envío a
